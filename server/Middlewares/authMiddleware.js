...
const protect = catchAsync(async (req, res, next) => {
	...
	if (!isJWT(token))
		return next(
			new AppError('Token is not valid use valid token or sign in again', 401)
		);

	if (!token) {
		return next(
			new AppError('You can not logged in. first log in and try again.', 403)
		);
	}
...
const signupMiddleware = async (req, res, next) => {
	...
	if (missingValues.length > 0)
		return next(new AppError(`requird missing values : ${missingValues}`, 400));
...
const signinMiddleware = async (req, res, next) => {
	...
	if (missingValue.length > 0)
		return next(new AppError(`requird missing values: ${missingValue}`, 400));
